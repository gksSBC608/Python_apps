...
class DBAccess():
  """
  This is data access class having all the methods to access database
  """
  def validate_login(self,username,password):
    ...
    try:
      ...
      if(1 <= all_recs.num_rows()):
        logging.info("login SQL query fetched "+str(all_recs.num_rows())+" records")
        result=True
      else:
        result= False
  ...
  def validateLogin1(self,username,password):
    return "Hello WOrld!"
  '''
  ...
  def fetch_account_detail(self,username):
    """
    This method fetches account detail for the currently logged in user based on his username
    """
    sql = "select firstname,lastname,account_number,account_balance from users,accounts where users.user_id = accounts.user_id and users.username='"+username+"'"
    ...
    try:
      ...
      rec = all_recs.fetch_row()
      #print("Number of elements in the record is:",len(rec))
      for firstname, lastname,account_no, account_balance in rec:
        fname= str(firstname,'utf-8')
        lname = str(lastname,'utf-8')
        #print(fname,lname,account_no,account_balance,sep='->')
        logging.debug("Account detail fetched is:"+str((fname,lname,account_no,account_balance)))
      return (fname,lname,account_no,account_balance)
  ...
  def addPayee1(self,payee):
    ...
    #logging.debug("Payee data to added is->"+str((firstname,lastname,payee_acc_no,payee_acc_bank))
    username = "username"
    password = "password"
    #sql_add_user = "insert into users values (null,'"+firstname+"','"+firstname+"','"+firstname+"','"+lastname+"')"
    ...
    try:
      ...
      logging.info("User added with ID "+str(user_id))
    except Exception as e:
      print("Database error while adding user =>",e)
      #logging.error("Database error while adding user, "+(e))
      result = False
    finally:
      conn.close()
    try:
      ...
      return False
    try:
      sql_add_account = "insert into accounts values("+str(payee_acc_no)+",1000.0,'"+payee_acc_bank+"',"+str(user_id)+")"
      ...
      #logging.error("Database error while adding account, "+(e))
      return False
    try:
      sql_add_payee = "insert into beneficiaries values(null,'"+payee_acc_name+"',"+str(payee_acc_no)+","+str(user_id)+")"
      ...
      #logging.error("Database error while adding payee, "+(e))
      result = False
    finally:
      conn.close()
    return result
  ...
  def deduct_balance(self,acc_number,username,amount):
    ...
    try:
      ...
      print("Database Error ->",e)
      logging.error("Database error while deducting balance, "+str(e))
    finally:
      conn.close()
    return result
  ...
  def get_payee_accounts(self,username):
    ...
    try:
      ...
      print("Database Error->",e)
      logging.error("Database error while getting beneficiaries, "+str(e))
  ...
  def get_payee_data(self,payee_acc_number):
    ...
    try:
      ...
      print("Database Error ->",e)
      logging.error("Database error while fetching payee data, "+str(e))
  ...
  def remove_payee(self,payee):
    ...
    try:
      ...
      print ("Connection Error ->", e)
      logging.error("Database error while removing payee, "+str(e))
  ...
  def deposit_money(self,username,acc_number,amount):
    ...
    sql = "update accounts set account_balance=account_balance+"+str(amount)+" where account_number="+str(acc_number)
    #print(sql)
    try:
      conn.query(sql)
      result = True
      ...
      print("Database Error ->",e)
      logging.error("Database error while deposit money, "+str(e))
    finally:
      conn.close()
    return result
  ...
  def check_if_user_registered(self,username):
    ...
    try:
      ...
      print("Database Error ->",e)
      logging.error("Database error while checking if user already exists, "+str(e))
      result = False
    finally:
      conn.close()
    return result     
...
#dba = DbAccess()
#isvalid = dba.validateLogin("gaurav","gaurav")
#print(isvalid)
